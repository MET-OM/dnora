...
class ForcingArome25thredds(ForcingFetcher):
    ...
    def __call__(self, grid, start_time, end_time, expansion_factor):
        ...
        msg.info(f"Getting wind forcing from Arome 2.5 km from {self.start_time} to {self.end_time}")
...
class ForcingMyWave3km(ForcingFetcher):
    ...
    def __call__(self, grid, start_time, end_time, expansion_factor):
        ...
        msg.info(f"Getting wind forcing from MEPS from {self.start_time} to {self.end_time}")
        for n in range(len(file_times)):
            #print(time_stamp)
            #print(days[n].strftime('%Y-%m-%d'))
    ...
    def get_url(self, time_stamp):
        ...
        url = 'https://thredds.met.no/thredds/dodsC/windsurfer/mywavewam3km_files/'+time_stamp.strftime('%Y')+'/'+time_stamp.strftime('%m')+'/'+filename
        return url
