...
class SWANInputFile(ModelInputFile):
    ...
    def __call__(self, start_time, end_time, swan_directory='.', wind=True, calib_wind=1, calib_wcap=0.5000E-04):
        ...
        with open(input_file, 'w') as file_out:
            ...
            file_out.write('READINP BOTTOM -1 \''+path_forcing
                           + self.grid.name()+'_SWAN.bot\' 3 0 FREE \n')
            file_out.write('$ \n')
            file_out.write('BOU NEST \''+path_forcing+self.grid.name()+'_spec'
                           + DATE_START.split('.')[0]+'_'+DATE_END.split('.')[0]+'.asc\' OPEN \n')
            file_out.write('$ \n')
            if wind == True:
                file_out.write('INPGRID WIND '+str(self.grid.lon()[0])+' '+str(self.grid.lat()[0])+' 0. '+str(self.forcing.nx()-1)+' '+str(self.forcing.ny()-1)+' '+str(
                    (delta_X/(self.forcing.nx()-1)).round(4)) + ' '+str((delta_Y/(self.forcing.ny()-1)).round(4)) + ' NONSTATIONARY ' + DATE_START + ' 1 HR ' + DATE_END + '\n')
                file_out.write('READINP WIND '+str(factor_wind)+'  \''+path_forcing+self.grid.name(
                )+'_wind'+DATE_START.split('.')[0]+'_'+DATE_END.split('.')[0]+'.asc\' 3 0 0 1 FREE \n')
                file_out.write('$ \n')
            else:
            ...
            file_out.write('BLOCK \'COMPGRID\' HEAD \''+path_forcing
                           + self.grid.name()+'_'+DATE_START.split('.')[0]+'.nc\' & \n')
            file_out.write(
                'LAY 1 HSIGN RTP TPS PDIR TM01 DIR DSPR WIND DEP OUTPUT ' + DATE_START + ' 1 HR \n')
            file_out.write('$ \n')
