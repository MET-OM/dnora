...
class OceanCurrentReader(ABC):
    """Reads ocean current forcing data from some source and provide it to the object.
...
class DnoraNc(OceanCurrentReader):
    ...
    def __call__(self, grid, start_time, end_time, expansion_factor):
        ...
        msg.info(f"Getting ocean_current OceanCurrent from cached netcdf (e.g. {self.files[0]}) from {start_time} to {end_time}")
...
class File_WW3Nc(OceanCurrentReader):
    def __init__(self, folder: str='', filename: str='ww3_ocean_current_T0', dateformat: str='%Y%m%dT%H%M', stride: int=None, hours_per_file: int=24, last_file: str='', lead_time: int=0) -> None:
        self.stride = stride
    ...
    def __call__(self, grid: Grid, start_time: str, end_time: str, expansion_factor: float):
        """Reads in all ocean_current data from a WW3 style ocean_current input"""
        ...
        msg.info(f"Getting ocean_current data from {self.filename} from {start_time} to {end_time}")
        ocr_list = []
        ...
        for time0, time1, file_time in zip(start_times, end_times, file_times):
            ...
            msg.plain(f"Reading ocean_current data: {time0}-{time1}")

            ocr_list.append(xr.open_dataset(filename).sel(time=slice(time0, time1),
