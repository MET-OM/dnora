...
class ModelRun:
    ...
    def import_forcing(self, forcing_reader: wnd.read.ForcingReader=None,
        ...
        time, u, v, lon, lat, x, y, attributes = forcing_reader(grid=self.grid(), start_time=self.time()[0], end_time=self.time()[-1], **kwargs)
        self._forcing = wnd.Forcing(lon=lon, lat=lat, x=x, y=y, time=time) 
