...
def ww3_grid(
    ...
    def write_depth():
        ...
        fout.write(
            f"  DEPTH%FILENAME         = {folder_on_server}{grid_exported_to[0]}\n"
        )
    ...
    def write_mask():
        fout.write("&MASK_NML\n")
        fout.write(
            f"  MASK%FILENAME         =  {folder_on_server}{grid_exported_to[-1]}\n"
        )
    ...
    def write_unst():
        ...
        fout.write(
            f"  UNST%FILENAME         =  {folder_on_server}{grid_exported_to[0]}\n"
        )
    ...
    with open("ww3_grid.nml", "w") as fout:
...
def ww3_prnc(
    filename: str, forcing_filename: str, folder_on_server: str, minwind: float = None
) -> None:
    ...
    def write_file():
        fout.write("&FILE_NML\n")
        fout.write(f"FILE%FILENAME      = '{folder_on_server}{forcing_filename}'\n")
    ...
    folder = __file__[:-6] + "/metadata/ww3_prnc/"

    with open("ww3_prnc.nml", "w") as fout:
        write_block("header.txt")
        fout.write("\n")
...
def ww3_specfile_list(list_of_filenames: list[str]):
    with open("spectral_boundary_files.list", "w") as fout:
        for fn in list_of_filenames:
            fout.write(f"{fn}\n")


def ww3_bounc(method: int, verbose_level: int):
    def write_block(fn: str):
        with open(f"{folder}{fn}", "r") as fin:
            block = fin.read()
        fout.write(block)
    ...
    def write_bound():
        ...
        fout.write("BOUND%MODE = 'spectral_boundary_files.list'\n")
        fout.write("/\n")

    folder = __file__[:-6] + "/metadata/ww3_bounc/"

    with open("ww3_bounc.nml", "w") as fout:
        write_block("header.txt")
        fout.write("\n")
        write_block("forcing.txt")
        write_bound()
...
def ww3_spectral_output_list(
    lons: np.ndarray, lats: np.ndarray, names: list[str] = None
):
    if names is None:
        names = [f"out_point{n+1:03.0f}" for n in range(len(lons))]
    with open("spectral_points.list", "w") as fout:
        for lon, lat, name in zip(lons, lats, names):
            fout.write(f"{lon:11.8f} {lat:11.8f} '{name}'\n")


def ww3_shel(start_time: str, end_time: str, homog_wind: tuple(float, float)):
    def write_block(fn: str):
        with open(f"{folder}{fn}", "r") as fin:
            block = fin.read()
        fout.write(block)
    ...
    def write_input():
        ...
        if homog_wind is None:
            fout.write("INPUT%FORCING%WINDS         = 'T'\n")
        else:
            fout.write("INPUT%FORCING%WINDS         = 'H'\n")
        fout.write("/\n")
    ...
    def write_output():
        ...
        fout.write("TYPE%POINT%FILE     = 'spectral_points.list'\n")
        fout.write("/\n")

    def write_date():
        fout.write("&OUTPUT_DATE_NML\n")
        start_time = {"FIELD": start_time, "POINT": start_time, "RESTART": start_time}
        end_time = {"FIELD": end_time, "POINT": end_time, "RESTART": end_time}
        dt = {"FIELD": "3600", "POINT": "3600", "RESTART": "86400"}
        for output_type in ["FIELD", "POINT", "RESTART"]:
            fout.write(
                f"DATE%{output_type}%START         = '{start_time[output_type]}'\n"
            )
            fout.write(
                f"DATE%{output_type}%STOP          = '{end_time[output_type]}'\n"
            )
    ...
    def write_homog():
        ...
        if homog_wind is not None:
            fout.write("  HOMOG_COUNT%N_WND    = 1\n")
        fout.write("/\n")
        fout.write("&HOMOG_INPUT_NML\n")
        if homog_wind is not None:
            fout.write("HOMOG_INPUT(1)%NAME       = 'WND'\n")
            fout.write(f"HOMOG_INPUT(1)%DATE   = {start_time}\n")
            fout.write(f"HOMOG_INPUT(1)%VALUE1     = {homog_wind[0]}\n")
            fout.write(f"HOMOG_INPUT(1)%VALUE2     = {homog_wind[1]}\n")
        fout.write("/\n")

    folder = __file__[:-6] + "/metadata/ww3_bounc/"

    with open("ww3_shel.nml", "w") as fout:
        write_block("header.txt")
        fout.write("\n")
