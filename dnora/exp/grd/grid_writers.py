...
class DnoraNc(GridWriter):
    def __call__(self, model: ModelRun, file_object: FileNames, extension='nc',**kwargs) -> tuple[str, str]:
        output_file = file_object.get_filepath(extension=extension)
        model.grid().ds().to_netcdf(output_file)
        print(output_file)
        return output_file
...
class Xyz(GridWriter):
    ...
    def __call__(self, model: ModelRun, file_object: FileNames, use_raw: bool=False, cartesian: bool=False, **kwargs) -> list[str]:
...
class REEF3D(GridWriter):
    ...
    def __call__(self, model: ModelRun, file_object: FileNames, use_raw: bool=False, **kwargs) -> str:
        filename = file_object.get_filepath()
