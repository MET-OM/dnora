...
class PowerSpectrum:
    def _reset_vars(self):
        self._set(np.zeros(self.size('all')),'spec')
        self._update_mask()

    def _update_mask(self) -> None:
        self._set(np.ones(self.size('grid')).astype(bool),'mask', only_grid_coords=True)
    ...
    def mask(self, logical=True) -> np.ndarray:
        ...
        mask = self._get('mask', default_data=np.ones(self.size('grid')).astype(int))
        if logical:
