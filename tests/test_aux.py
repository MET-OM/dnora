...
sys.path.insert(0, "../../")
from dnora.aux_funcs import create_time_stamps, u_v_from_speed_dir, speed_dir_from_u_v
import numpy as np
...
class WindDir(unittest.TestCase):
    def test_u_v(self):
        u = np.array([0, 0, 0, 1, 1, 1, -1, -1, -1])
        v = np.array([0, 1, -1, 0, 1, -1, 0, 1, -1])
        ws, wdir = speed_dir_from_u_v(u, v)
        np.testing.assert_array_almost_equal(
            ws, np.array([0, 1, 1, 1, 2**0.5, 2**0.5, 1, 2**0.5, 2**0.5])
        )
        np.testing.assert_array_almost_equal(
            wdir, np.array([270, 180, 0, 270, 225, 315, 90, 135, 45])
        )
    ...
    def test_circular(self):
        ...
        v = np.array([0, 1, -1, 0, 1, -1, 0, 1, -1])
        ws, wdir = speed_dir_from_u_v(u, v)
        u1, v1 = u_v_from_speed_dir(ws, wdir)
        np.testing.assert_array_almost_equal(u1, u)
        np.testing.assert_array_almost_equal(v1, v)

    def test_main_directions(self):
        ws = 1
        u, v = u_v_from_speed_dir(ws, wdir=0)
        self.assertAlmostEqual(u, 0)
        self.assertAlmostEqual(v, -ws)

        u, v = u_v_from_speed_dir(ws, wdir=90)
        self.assertAlmostEqual(u, -ws)
        self.assertAlmostEqual(v, 0)

        ws = 2
        u, v = u_v_from_speed_dir(ws, wdir=180)
        self.assertAlmostEqual(u, 0)
        self.assertAlmostEqual(v, ws)

        u, v = u_v_from_speed_dir(ws, wdir=270)
        self.assertAlmostEqual(u, ws)
        self.assertAlmostEqual(v, 0)
