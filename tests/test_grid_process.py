...
def test_min_depth():
    grid = Grid(lon=(1, 2), lat=(0, 3))
    grid.set_spacing(nx=10, ny=10)
    grid.process_grid(SetMinDepth(depth=1000.0))
    np.testing.assert_array_almost_equal(grid.topo().mean(), 1000.0)
    grid.process_grid(SetMinDepth(depth=10.0))
    np.testing.assert_array_almost_equal(grid.topo().mean(), 1000.0)
...
def test_min_depth():
    grid = Grid(lon=(1, 2), lat=(0, 3))
    grid.set_spacing(nx=10, ny=10)
    grid.process_grid(SetMaxDepth(depth=100.0))
    np.testing.assert_array_almost_equal(grid.topo().mean(), 100.0)
    grid.process_grid(SetMaxDepth(depth=1000.0))
    np.testing.assert_array_almost_equal(grid.topo().mean(), 100.0)
