...
def test_trivial_picker(grid, all_points):
    inds = bnd.pick.TrivialPicker()(grid, all_points)
    np.testing.assert_array_almost_equal(inds,all_points.inds())

def test_area_picker(grid, all_points):
    small_grid = grd.Grid(lon=(15,16), lat=(62,63))
    inds = bnd.pick.Area()(small_grid, all_points, expansion_factor=1.0)
...
def test_area_picker_expand(all_points):
    small_grid = grd.Grid(lon=(15,16), lat=(62,63))
    inds = bnd.pick.Area()(small_grid, all_points, expansion_factor=2.0)
...
def test_selected_points(grid, all_points):
    selected_points = PointSkeleton.from_skeleton(all_points, mask=all_points.lon()<10.01)
    inds = bnd.pick.NearestGridPoint()(grid, all_points, selected_points)
